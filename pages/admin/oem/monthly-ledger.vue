<template>
  <div class="monthly-ledger-page">
    ...
    >
      <template #actions>
        <button class="btn-action" @click="loadLedger" :disabled="loading">
    ...
    <div class="content-section">
      ...
      <!-- 지급 상태 카드 -->
      <div v-if="ledgerData" class="payment-status-card" :class="paymentStatusClass">
      ...
      <div v-else-if="!ledgerData || ledgerData.items.length === 0" class="no-data-message">
        ...
        <p v-if="!selectedOemCompanyId">OEM 제조사를 선택한 후 조회하세요.</p>
        <p v-else>해당 월에 발주 데이터가 없습니다.</p>
      </div>
          ...
          <table class="data-table ledger-table">
            ...
            <tbody>
              <tr v-for="(item, index) in ledgerData.items" :key="index">
                ...
                <td class="cell-project">{{ item.projectName }}</td>
                <td class="cell-spec">{{ item.spec }}</td>
                <td class="text-right">{{ formatQuantity(item.quantity) }}</td>
                <td class="text-right cell-amount">{{ formatCurrency(item.unitCost) }}</td>
            ...
            <tfoot>
              <tr class="total-row">
                ...
                <td class="text-right"><strong>{{ formatQuantity(ledgerData.totalQuantity) }}</strong></td>
                <td></td>
                <td class="text-right"><strong>{{ formatCurrency(ledgerData.totalAmount) }}</strong></td>
                <td></td>
              </tr>
            </tfoot>
          </table>
        ...
        <div class="modal-container modal-sm">
          ...
          <div class="modal-body">
            <div class="form-group">
              <label>지급 금액</label>
              <input type="number" v-model.number="completeForm.paidAmount" class="form-input" placeholder="실제 지급 금액">
            </div>
            <div class="form-group">
              <label>지급일</label>
              <input type="date" v-model="completeForm.paidDate" class="form-input">
            </div>
          </div>
          <div class="modal-footer">
            <button class="btn-action" @click="showCompleteModal = false">취소</button>
            <button class="btn-action btn-primary" @click="handleComplete" :disabled="!completeForm.paidAmount">
              지급 완료
            </button>
          </div>
        </div>
...
onMounted(async () => {
  try {
    ...
    // OEM 담당자는 자사 companyId로 자동 선택 + 잠금
