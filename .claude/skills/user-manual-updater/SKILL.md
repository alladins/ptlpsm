...
description: |
  PTLPSM 시스템의 권위 있는 도메인 레퍼런스 — 엔티티 관계도, 6개 도메인 상태흐름(출하·운송·납품확인·
  기성차수·자금·결제), 자금흐름(선급금·기성금·잔금), 서류 생성 타이밍, 비즈니스 규칙, 용어/상태 한글표기를
  담는다. 매뉴얼을 작성·갱신하거나 도메인 사실(상태 전이·금액 계산·서명 규칙)을 확인해야 할 때 이 스킬을
  로드할 것. 문서 생성 결과의 사실 검증 기준으로도 사용한다.
---
