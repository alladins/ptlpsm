...
화면별 사용법(2장 이후)을 작성·보강할 때는 각 기능 절에 다음 4가지를 **실제 코드 근거로** 채운다. 추측 금지 — 코드에서 확인 못한 UI/값은 `(확인 필요)`로 표기한다. 독자는 비개발자 실무 사용자이므로 운영/조작 관점으로 쓴다.

1. **단계별 조작 가이드** — 번호 매긴 조작 순서. 실제 버튼/메뉴 라벨(`<button>`·FormField label)과 화면 이동, 실제 라우트(`/admin/...`)를 명시.
...
**스크린샷 자리표시 규칙**: 화면 설명마다 `> 📷 [스크린샷: <화면명> — <무엇을 보여주는지>]` 형식으로 삽입(추후 실제 캡처 교체용).
