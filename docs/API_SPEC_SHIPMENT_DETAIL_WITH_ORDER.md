...
| 항목 | 상태 | 완료일 | 비고 |
|------|------|--------|------|
| **API 스펙 문서** | ✅ 완료 | 2025-01-25 | 본 문서 |
| **프론트엔드 적용** | ✅ 완료 | 2025-01-25 | [shipping/edit/[id].vue](../pages/admin/shipping/edit/[id].vue) |
| **백엔드 API 구현** | 🟡 대기 중 | - | Java/Spring Boot 구현 필요 |
| **통합 테스트** | ⏳ 대기 중 | - | 백엔드 완료 후 진행 |
| **프로덕션 배포** | ⏳ 대기 중 | - | 테스트 완료 후 진행 |

### 구현 체크리스트

**백엔드 (Spring Boot):**
- [ ] `ShipmentController`에 새 엔드포인트 추가
- [ ] `ShipmentService`에 통합 조회 로직 구현
- [ ] SQL JOIN 쿼리 작성 및 최적화
- [ ] DTO 매핑 (응답 객체 생성)
- [ ] 에러 처리 (shipmentId 없음, orderId 없음)
- [ ] 단위 테스트 작성
- [ ] API 문서 업데이트 (Swagger/OpenAPI)

**테스트:**
- [ ] Postman/cURL로 API 호출 테스트
- [ ] 프론트엔드 연동 테스트 (실제 데이터 조회)
- [ ] 성능 테스트 (응답 시간 측정)
- [ ] 에러 시나리오 테스트
